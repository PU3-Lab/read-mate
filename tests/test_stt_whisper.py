...
def make_mock_transcribe_result(text: str = "안녕하세요 테스트입니다", language: str = "ko"):
...
def test_run_pipeline_returns_stt_result(stt, tmp_path):
    ...
    with patch.object(stt, "_extract_from_youtube", return_value=str(fake_wav)), \
...
def test_run_pipeline_returns_none_on_error(stt):
    ...
    with patch.object(stt, "_extract_from_youtube", side_effect=Exception("다운로드 실패")):
