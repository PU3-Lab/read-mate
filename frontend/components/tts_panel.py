...
def render_tts_panel():
	...
	function claimAudio(){{
	  ...
	  try{{
	    ...
	    owner.__rmCurrentAudio=a;
	  }}catch(err){{}}
	}}
	function onKey(e){{
	  const tag=e.target.tagName;
	  if(tag==='INPUT'||tag==='TEXTAREA')return;
	  if(e.code==='Space'){{e.preventDefault();claimAudio();a.paused?a.play():a.pause();}}
	  if(e.key.toLowerCase()==='r'){{claimAudio();a.currentTime=0;a.play();}}
	}}
	a.addEventListener('play',claimAudio);
	claimAudio();
	a.currentTime=0;
	a.play().catch(()=>{{}});
